...
aws emr create-cluster --profile ${PROFILE} --region ${REGION} --name ${CLUSTER_NAME} --ami-version ${AMI_VSN} --instance-type ${INSTANCE_TYPE} --instance-count ${INSTANCE_COUNT} \
  --log-uri ${LOG_URL} \
  --enable-debugging \
  --ec2-attributes KeyName=${KEYNAME} --applications Name=Hive Name=GANGLIA \
  --bootstrap-actions Name=Install_Spark,Path=s3://support.elasticmapreduce/spark/install-spark \
